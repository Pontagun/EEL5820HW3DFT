...
if __name__ == "__main__":
    ...
    # dft_img = dft(img, rows, cols)

    # image = np.fft.fft2(centered_image)
    image = dft(centered_image, rows, cols)
    image = abs(image)
    norm_image = pixel_normalization(image)
    # image = abs(image)
    # print("Library's Magnitude")
    # print(image)

    # if dft_img.all == image.all:
    #     print("Equal")
    # else:
    #     print("------------")
    #     print(dft_img - image)
